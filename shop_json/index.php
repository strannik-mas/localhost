<?php
//программа работает для версий php >=5.5 (метод SplFileObject::fread)
...
class FileExtensionFinder extends FilterIterator {
	...
	public function accept() {
		...
		//функция, удаляющая пустые значения и лишние пробелы
	...
	function build($path, $jarr, $names){
		if (explode("\\",$path)[3] == 'clothes')
			$realpath = DIR_PREFIX ."\\". explode("\\",$path)[3] . "\\" . $jarr['clothes']['name'] ."\\". $names[(explode("\\",$path)[2])-1];
		else $realpath = DIR_PREFIX ."\\". explode("\\",$path)[1] . "\\" . $jarr['name'];
		...
		if($jarr['images']) {
			...
			for($i=0; $i<count($pathimages); $i++){
				...
				$jarr['images'][$i]['new'] = str_replace("\\", "/", $newfile);
				$jarr['images'][$i]['thumb'] = str_replace("\\", "/", copyImage($pathimages[$i], substr($newfile, 0, -4)));
			}
		}
		...
		file_put_contents(rus2translit($realpath) . '\\main.json', 
					json_encode($jarr, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
	...
	foreach($filepath as $k=>$v){
		...
		$data[$k] = json_decode(preg_replace($pattern, $replacement, $string), true);
		
	}
	
	//плучаем массив массивов с удалёнными лишними пробелами и удаленными пустыми данными
	$newdata = array_item_trim($data);		
	
		//найдем названия магазинов
	foreach($newdata as $d){
		if($d['contacts']) $shop_names[] = $d['name'];
	}
	
		//создаём новую структуру если ее еще нет
	if(!file_exists(DIR_PREFIX)){
		foreach($newdata as $k=>$v){
			build($k, $v, $shop_names);
		}
